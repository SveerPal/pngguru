...
@section('content')
    ...
    <div class="row user">       
        <div class="col-md-12">
            <a href="{{ route('admin.image-categories') }}" class="btn btn-primary text-white mr-1 mb-4" type="button">Back To Image Category</a>
            <form action="{{ route('admin.image-categories.store') }}" method="POST" role="form" enctype="multipart/form-data">
                @csrf
                <div class="tile">  
                    <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control" id="parent" name="parent" value="{{ old('parent') }}"/>
                                ...
                                @foreach($parentImage_categories as $image_category)                                
                                    <option value="{{ $image_category->id }}">{{ $image_category->name }}</option>
                                @endforeach 
                            </select>
                            @error('parent')
                                <div class="alert alert-danger error">{{ $message }}</div>
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <textarea class="form-control" rows="4" placeholder="Enter  description" id="description"  name="description" >{{ old('description') }}</textarea>
                            @error('description')
                                <div class="alert alert-danger error">{{ $message }}</div>
                            @enderror    
                        </div>
                        <div class="form-group col-md-12">
                            <div class="form-group">
                                <div class="form-check">
                                   ...
                                   <input class="form-check-input" type="checkbox" id="banner_search_below" name="banner_search_below" value="1" />Banner Search Below
                                   </label>
                                </div>
                            </div>
                            <div class="form-group">
                                <div class="form-check">
                                   <label class="form-check-label">
                                   <input class="form-check-input" type="checkbox" id="popular" name="popular" value="1" />Popular
                                   </label>
                                </div>
                            </div>
                        </div>    
